...
namespace ShapeKeyTools
{
    public partial class ShapeKeyToolWindow
    {
        ...
        {
            GroupShapesInternal(null);
        }

        internal void GroupShapesPreserveLoadedGroups(Dictionary<string, List<BlendShape>> loadedGroups)
        {
            GroupShapesInternal(loadedGroups);
        }

        private void GroupShapesInternal(Dictionary<string, List<BlendShape>> loadedGroups = null)
        {
            // 読み込んだグループ情報がある場合は、通常のグループ化処理をスキップ
            if (loadedGroups != null && loadedGroups.Count > 0)
            {
                ...
                {
                    groupedShapes[loadedGroup.Key] = new List<BlendShape>(loadedGroup.Value);
                }
                return;
            }
            ...
            groupedShapes.Clear();
            // groupFoldouts.Clear(); // 既存の展開状態を保持するため削除
            currentGroup = "その他";
    ...
    internal static class Grouping
    {
        internal static void ApplyTestSliderToGroup(ShapeKeyToolWindow window, string groupName, int newIndex, bool skipNonZero)
        {
            ...
            for (int i = 0; i < shapes.Count; i++)
            {
                ...
                ShapeKeyCommandService.SetBlendShapeWeightImmediate(window, shapes[i], targetWeight);
            }
        ...
        internal static void ApplyTestSliderToVisibleShapes(ShapeKeyToolWindow window, string groupName, List<BlendShape> visibleShapes, int newIndex, bool skipNonZero)
        {
            ...
            for (int i = 0; i < visibleShapes.Count; i++)
            {
                ...
                ShapeKeyCommandService.SetBlendShapeWeightImmediate(window, visibleShapes[i], targetWeight);
            }
