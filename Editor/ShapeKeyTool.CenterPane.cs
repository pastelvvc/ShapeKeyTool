...
namespace ShapeKeyTools
{
    ...
    internal class CenterPane
    {
        ...
        private void DrawTreeViewSearchUI(Rect rect)
        {
            ...
            {
                UIEventHandlers.OnTreeSearchTextChanged(window, newSearchText);
            }

            // クリアボタン
            if (GUILayout.Button("クリア", GUILayout.Width(50)))
            {
                UIEventHandlers.OnTreeSearchClear(window);
            }
            EditorGUILayout.EndHorizontal();
            ...
            // 正規表現が無効な場合の警告（UI統一）
            if (SearchManager.useRegex && !string.IsNullOrEmpty(SearchManager.treeViewSearchText) && !SearchManager.IsValidRegex(SearchManager.treeViewSearchText))
            {
