                ...
                switch (window.operationMenuIndex)
                {
                    case 1: // ランダム設定
                        foreach (var group in window.groupedShapes)
                        {
                            foreach (var shape in group.Value)
                            {
                                ...
                                shape.weight = randomWeight;
                                window.selectedRenderer.SetBlendShapeWeight(shape.index, randomWeight);
                            }
                        }
                    ...
                    case 3: // すべてアンロック
                        foreach (var group in window.groupedShapes)
                        {
                            foreach (var shape in group.Value)
                            {
                                ...
                                window.lockedShapeKeys[shape.index] = false;
                            }
                        }
                        EditorUtility.SetDirty(window.selectedRenderer);
                        TreeViewPart.Reload();
                    ...
                    case 4: // すべてリセット
                        foreach (var group in window.groupedShapes)
                        {
                            ...
                            {
                                shape.weight = 0f;
                                window.selectedRenderer.SetBlendShapeWeight(shape.index, 0f);
                            }
                        }
                        EditorUtility.SetDirty(window.selectedRenderer);
                        window.operationMenuIndex = 0;
