    ...
    internal static class TreeViewPart
    {
        ...
        private class ShapeKeyTreeView : TreeView
        {
            ...
            private void MoveGroupToEnd(string groupName)
            {
                ...
                tool.groupedShapes = newGroups;
                tool.groupFoldouts = newFoldouts;
                tool.groupTestSliders = newTestSliders;
            ...
            protected override void ContextClickedItem(int id)
            {
                ...
                if (item != null)
                {
                    ...
                    else if (item.depth == 1)
                    {
                        ...
                        // 拡張パラメータが既に存在する場合は削除メニューも追加
                        var extendedInfo = ExtendedShapeKeyInfo.TryParseFromName(currentName, out var info);
                        if (extendedInfo)
                        {
                            menu.AddItem(new GUIContent("拡張パラメータを削除"), false, () =>
                            {
                                RemoveExtendedParameters(currentName);
                            });
                        }
            ...
            private void RemoveExtendedParameters(string shapeKeyName)
            {
                try
                {
                    // 拡張パラメータを解析
                    ...
                    bool shouldRemove = EditorUtility.DisplayDialog(
                        "拡張パラメータ削除の確認",
                        $"シェイプキー '{shapeKeyName}' から拡張パラメータを削除しますか？\n\n" +
                        $"現在の範囲: {info.minValue} 〜 {info.maxValue}\n" +
                        $"元の名前: {info.originalName}",
                        "削除",
                        "キャンセル"
                    );
                    ...
                    foreach (var group in tool.groupedShapes)
                    {
                        for (int i = 0; i < group.Value.Count; i++)
                        {
                            ...
                            if (shape.name == shapeKeyName)
                            {
                                ...
                                // 名前を元の名前に戻す
                                shape.name = info.originalName;
                                
                                // 重みを0-100の範囲に制限
                                if (shape.weight < 0f) shape.weight = 0f;
                                if (shape.weight > 100f) shape.weight = 100f;
            ...
            private void DeleteShapeKey(string shapeKeyName)
            {
                ...
                try
                {
                    ...
                    // 永続化データを更新
                    if (ShapeKeyPersistenceManager.AutoSaveEnabled && tool.selectedRenderer != null)
                    {
                        var target = tool.selectedRenderer.gameObject;
                        var persistence = ShapeKeyPersistenceManager.GetOrCreateComponent(target);
                        ShapeKeyPersistenceEditor.SaveData(persistence, tool);
                    }
